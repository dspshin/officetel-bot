...
url = 'http://openapi.molit.go.kr:8081/OpenAPI_ToolInstallPackage/service/rest/RTMSOBJSvc/getRTMSDataSvcRHTrade?ServiceKey='+key
...
def help(id):
    sendMessage(id, '''연립다세대 매매 정보 알림용 텔레그램 봇입니다.
명령어 사용법:
...
def howmuch(loc_param, date_param, filter_param):
    ...
    for item in items:
        try:
            ...
            row=''
            row += office['년']+'/'+office['월']+'/'+office['일']+', '+office['법정동']+' '+office['지번']+' '+office['연립다세대']+', '+office['층']+'F, '+office['전용면적']+'m², '+office['거래금액']+'만원\n'
        except:
            print str(datetime.now()).split('.')[0]
            traceback.print_exc(file=sys.stdout)
